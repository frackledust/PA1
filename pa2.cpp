...
double ithPermutation(const int n, int i, std::vector <int> &fact)
{
    int j, k = 0;
...
//        printf("%d ", perm[k]);
//    printf("\n");
    double dist = get_path_distance(perm);
...
//#pragma omp parallel for default(none) shared(n, results, f, fact)
//    for (int i = 0; i < f; i++) {
//        results[i] = ithPermutation(n, i, fact);
//    }

//#pragma omp parallel for default(none) private(f) shared(permutation, results)
    for(int i = 0; i < f; i++) {
